...
public class TesteConexao extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
           ...
           DQuery conn = new DQuery();
           String[] valores = {"00", "Vinicius"};
            try {
                //out.print(conn.insert("usuarios", valores));
                conn.insert("usuarios", valores);
                out.print("Usuario inserido com sucesso!!");
            } catch (SQLException ex) {
                out.print("Erro ao inserir usuario" +  ex);
